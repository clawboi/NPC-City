...
export class NPCSystem {
  ...
  spawnActorDistrict(){
    ...
    for (let i=0; i<names.length; i++){
      ...
      const homes = (this.world && this.world.homes) ? this.world.homes : [];
      if (homes.length){
        // stable distribution so each NPC has a "home"
        n.homeDoor = homes[i % homes.length].door;
        n.homeId = homes[i % homes.length].id;
      } else {
        n.homeDoor = null;
        n.homeId = null;
      }
      // personal offset so NPCs don't stack perfectly on the same point
      n.offX = (rand(this)*18 - 9);
      n.offY = (rand(this)*18 - 9);
...
if (dayKey && n._dayKey !== dayKey){
  ...
  if (n.state !== "inside") n.state = "roam";
      ...
      if (n.state === "go" && n.goal){
        const gx = n.goal.x + (n.offX||0), gy = n.goal.y + (n.offY||0);
        ...
        if (d < 18){
          ...
          // separation: prevents conga-lines (cheap repulsion)
          let sx = 0, sy = 0, ct = 0;
          for (let j=0; j<this.list.length; j++){
            if (j===i) continue;
            const o = this.list[j];
            const ox = (o.x + o.w*0.5) - (n.x + n.w*0.5);
            const oy = (o.y + o.h*0.5) - (n.y + n.h*0.5);
            const dd = ox*ox + oy*oy;
            if (dd > 0.0001 && dd < 34*34){
              const inv = 1/Math.sqrt(dd);
              const push = (34 - Math.sqrt(dd)) * 0.22;
              sx -= ox*inv * push;
              sy -= oy*inv * push;
              ct++;
            }
