  ...
  renderOverlay(){
    ...
    if (this.overlayTab === "inventory"){
      ...
      const iconFor = (type)=>{
        ...
        if (type.includes("bat")) return "🏏";
        if (type.includes("pistol") || type.includes("gun")) return "🔫";
        if (type.includes("phone")) return "📱";
        if (type.includes("keys")) return "🗝️";
        if (type.includes("mushroom")) return "🍄";
      ...
      const slots = (inv.slots && inv.slots.length) ? inv.slots : [null,null,null];
      const active = (typeof inv.active === "number") ? inv.active : 0;
      const outfits = (inv.outfits||[]);

      body.innerHTML = `
        <div class="sec">
          <div class="sec-title">Inventory</div>
          <div class="muted">Hotkeys: 1-3 equip • 0 empty • Q cycle</div>
          ...
          <div class="inv-grid">
            ${slots.map((it,idx)=>{
              ...
              const on = (active === idx);
              return `
                <div class="inv-card ${on?"on":""}">
                  <div class="inv-ic">${iconFor(t)}</div>
                  <div class="inv-meta">
                    <div class="inv-title">Slot ${idx+1}</div>
                    <div class="inv-name">${pretty(t)}</div>
                  </div>
                  <div class="inv-actions">
                    <button class="btn sm ${on?"primary":""}" data-equip="${idx}">${on?"Equipped":"Equip"}</button>
                  </div>
                </div>
              `;
            }).join("")}

            <div class="inv-card ${(active<0)?"on":""}">
              <div class="inv-ic">✋</div>
              <div class="inv-meta">
                <div class="inv-title">Hands</div>
                <div class="inv-name">Empty</div>
              </div>
              <div class="inv-actions">
                <button class="btn sm ${(active<0)?"primary":""}" data-equip="-1">${(active<0)?"Equipped":"Empty"}</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="sec">
          ...
          <div class="muted">Cosmetic for now. We’ll make outfits affect stats later.</div>
          <div class="chips">
            ${outfits.map((o)=>{
              const on = (o.id === inv.outfit);
              return `<button class="chip ${on?"on":""}" data-wear="${o.id}">${o.label}</button>`;
            }).join("")}
          </div>
        </div>
      `;
