...
export class Game {
  constructor({ canvas, ctx, input, save, ui, assets, world }){
    ...
    this.homesTaken = Array(6).fill(false);
...
this.inv.applyToPlayer(this.player); // ensures player.held exists
  ...
  continueGame(){
    ...
    // housing
    this.homesTaken = Array.isArray(data.homesTaken) ? data.homesTaken.slice(0,6) : Array(6).fill(false);
    if (this.homesTaken.length < 6) this.homesTaken = this.homesTaken.concat(Array(6-this.homesTaken.length).fill(false));
    if (this.player.homeId == null) this.player.homeId = null;
  ...
  update(dt){
    ...
    // HOME mode: stop world sim, allow exit + passive recovery
    if (this.place === "home"){
      // allow overlay toggle
      if (this.input.pressed("e")){
        // exit back to street at door
        this.place = "street";
      ...
      // passive recovery (gentle)
      this.player.sleep = Math.min(100, this.player.sleep + dt*6);
      this.player.hygiene = Math.min(100, this.player.hygiene + dt*4);
      this.player.health = Math.min(this.player.healthMax, this.player.health + dt*1.5);
      return;
    }
...
if (ax !== 0 || ay !== 0){
  ...
  this.player.faceY = ay;
    ...
    if (nearNPC){
      ...
      let prompt = "E · " + lm.text + "  ·  " + (lm.hint || "Interact");
      if (lm.id === "apt"){
        const id = lm.homeId|0;
        const home = (this.world.homes||[]).find(h=>h.id===id);
        const price = home ? home.price : (220 + id*40);
        const taken = !!this.homesTaken[id];
        if (this.player.homeId === id){
          prompt = "E · Enter " + (home?home.label:("Apt "+(id+1))) + "  ·  Home";
        } else if (this.player.homeId == null){
          prompt = taken
            ? ((home?home.label:("Apt "+(id+1))) + "  ·  TAKEN")
            : ("E · Buy " + (home?home.label:("Apt "+(id+1))) + "  ·  $" + price);
        } else {
          prompt = (home?home.label:("Apt "+(id+1))) + "  ·  Not yours";
        }
      }
      if (lm.id === "vault"){
        prompt = "E · Management  ·  Vault / Office";
      }
      if (this.ui.setPrompt) this.ui.setPrompt(prompt);
  ...
  handleInteract(lm){
    ...
    switch (lm.id){
      ...
      case "apt": {
        ...
        const home = (this.world.homes||[]).find(h=>h.id===id);
        const price = home ? home.price : (220 + id*40);
        const label = home ? home.label : ("Apt " + (id+1));
        const taken = !!this.homesTaken[id];

        if (this.player.homeId === id){
          this.place = "home";
          this._streetReturn = { x: (home?.door?.x ?? lm.x) - this.player.w*0.5, y: (home?.door?.y ?? lm.y) - this.player.h*0.5 };
          this.toast(label + " · Entered");
          break;
        }

        if (this.player.homeId == null){
          if (taken){
            this.toast(label + " is taken.");
            break;
          }
          if (this.player.money < price){
            this.toast("Need $" + price + " to buy " + label + ".");
            break;
          }
          this.player.money -= price;
          this.player.homeId = id;
          this.homesTaken[id] = true;
          this.toast("Bought " + label + " for $" + price + ".");
          break;
        }

        this.toast("You already own a home. (Sell/swap later.)");
        break;
      }
